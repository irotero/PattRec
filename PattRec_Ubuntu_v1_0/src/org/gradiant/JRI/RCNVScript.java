...
public class RCNVScript implements RMainLoopCallbacks {
	...
	"set.seed(1)\n" +
        ...
        "setwd(wd)\n" +
        "#------------------------------------------------------------------------------\n" +
	"ppal <- function(fvcf, per, per_gene, cov_cont, name_test, name_cont, name_fasta, excel, genes, polymorphic, poly_regions, do_plot, restr, fixed, down){\n" +
	"	lectura <- rd(name_test, name_cont, name_fasta, fvcf, polymorphic, poly_regions, fixed)\n" +
	"	name_cont <- lectura$name_cont\n" +
	"	name_cont <- gsub(\".bam\", \"\", name_cont)\n" +
	"	name_test <- gsub(\".bam\", \"\", name_test)\n" +
	...
	"	output <- concat(output,cov_cont,per)\n" +
	"	nexons <- norm_exon(lectura$n, normalized$test, normalized$cont, listgenes, output)\n" +
	"	residuos <- regression(lectura$n, nexons)\n" +
	"	output1 <- filt(output, per, normalized$cont, lectura$n, residuos, sex, restr, cov_cont)\n" +
	"	if(down==TRUE & nrow(output1)>2) output1 <- downsampling(name_test, name_cont, lectura$n, output1, cov_cont, lectura$gc, per, sex, restr)\n" +
	"	output <- output1\n" +
	...
	"	}\n" +
	"	if(fvcf!=\"NO\" && length(fvcf)!=0 && class(output_genes)!=\"NULL\" && nrow(output_genes)>0) output_genes <- vcf_filter(output_genes, lectura$vcf, cov_cont)\n" +
	...
	"}\n" +
        "#------------------------------------------------------------------------------\n" +
        "rd <- function(name_test, name_cont, name_fasta, fvcf, polymorphic, poly_regions, fixed){\n" +
        "	gc <- read.table(\"GC_TestSample.txt\", header=F, sep=\"\\t\", stringsAsFactors=F)\n" +
        "	gc <- gc[,-5]\n" +
        "	gc <- gc[,-(6:ncol(gc))]\n" +
        "	gc[,1] <- toupper(gc[,1])\n" +
        "	gc[,1] <- gsub(\"CHR\",\"chr\",gc[,1])\n" +
        "	if(length(grep(\"chr\",gc[,1]))==0) gc[,1] <- paste0(\"chr\",gc[,1])\n" +
        "	test <- read.table(paste0(name_test,\".txt\"), header=F, sep=\"\\t\", stringsAsFactors=F)\n" +
	...
	"	n <- length(name_cont)\n" +
        ...
        "	for(i in 1:n){\n" +
        "		cnt <- read.table(paste0(name_cont[i],\".txt\"), header=F, sep=\"\\t\", stringsAsFactors=F)\n" +
        "		assign(paste0(\"cont\",i),cnt)\n" +
        "		aux.cont[,i] <- cnt[,6]\n" +
        "	}\n" +
	"	if(n>=2 & fixed==F){\n" +
	...
	"		}\n" +
	"	}\n" +
        "       aux.cont <- as.data.frame(aux.cont)\n" +
	...
	"	if(length(grep(\"chr\",cont$chr))==0) cont$chr <- paste0(\"chr\",cont$chr)\n" +
        ...
        "	}\n" +
        "	if(fvcf!=\"NO\"){\n" +
        "		vcf <- read.table(paste0(name_test,\".vcf\"), header=F, sep=\"\\t\", stringsAsFactors=F)\n" +
	...
	"		return(list(\"n\"=n, \"test\"=test, \"cont\"=cont, \"gc\"=gc, \"vcf\"=vcf, \"name_cont\"=name_cont))\n" +
        "	}\n" +
	"	if(fvcf==\"NO\") return(list(\"n\"=n, \"test\"=test, \"cont\"=cont, \"gc\"=gc, \"name_cont\"=name_cont))\n" +
        "}\n" +
        "#------------------------------------------------------------------------------\n" +
	...
	"	output1 <- output1[which((as.numeric(output1$End)-as.numeric(output1$Start))>0),]\n" +
	"	return(output1)\n" +
	"}\n" +
        "#------------------------------------------------------------------------------\n" +
	...
	"	if(summary(mod)$adj.r.squared>0.8){\n" +
	"		aux[,7] <- mod$residuals\n" +
	...
	"}\n" +
        ...
        "}\n" +
        "#------------------------------------------------------------------------------\n" +
	"downsampling <- function(name_test, name_cont, n, output1, cov_cont, gc, per, sex, restr){\n" +
	"	outputdwn <- list()\n" +
	"	for(k in 1:n){\n" +
	"		aux.cont <- read.table(paste0(name_cont[n],\"_dwn.txt\"), header=F, sep=\"\t\", stringsAsFactors=F)\n" +
	"		names(aux.cont) <- c(\"chr\",\"start\",\"end\",\"gen\",\"pb\",\"cov\")\n" +
	"		if(k==1){\n" +
	...
	"		test <- read.table(paste(c(name_test, \"_dwn\", i, \".txt\"), collapse=\"\"), header=F, sep=\"\t\", stringsAsFactors=F)\n" +
	"		names(test) <- c(\"chr\",\"start\",\"end\",\"gen\",\"pb\",\"cov\")\n" +
	"		test$gen <- toupper(test$gen)\n" +
	"		test$chr <- toupper(test$chr)\n" +
	"		test$chr <- gsub(\"CHR\",\"chr\",test$chr)\n" +
	"		if(length(grep(\"chr\",test$chr))==0) test$chr <- paste0(\"chr\",test$chr)\n" +
        ...
        "			l <- which(gc[,4]==output1$Gen[i] & (gc[,2]>output1$End[i] | gc[,3]<output1$Start[i]))\n" +
        "			if(length(l)==0) eldwn <- c(eldwn,i)\n" +
        "			if(length(l)!=0){\n" +
        "				aux.dwn <- outputdwn[outputdwn$Gen==output1$Gen[i] & outputdwn$Start<=max(gc[l,3]) & outputdwn$End>=min(gc[l,2]),]\n" +
	...
	"}\n" +
        ...
        "	dbPrueba<-dbConnect(MySQL(), user=userDB, password=passwordDB, host=\"127.0.0.1\", dbname=\"CNV\")\n" +
        "	gr <- output[,1:5]\n" +
	...
	"	if(genes==T){\n" +
	"		write.table(output_genes, file = paste0(paste(name_test,paste(paste(name_cont,collapse=\"-\"), \"gen\", sep=\"_\"), sep=\"_\"), \".txt\"), append = FALSE, quote = TRUE, sep = \"\\t\",eol = \"\\n\", na = \"\", dec = \",\", row.names = F,col.names = TRUE, qmethod = c(\"escape\", \"double\"),fileEncoding = \"\")\n" +
	"	}\n" +
	"	write.table(info, file = paste0(paste(name_test,paste(paste(name_cont,collapse=\"-\"), \"information\",sep=\"_\"), sep=\"_\"),\".txt\"), append = FALSE, quote = TRUE, sep = \"\\t\",eol = \"\\n\", na = \"\", dec = \",\", row.names = F,col.names = TRUE, qmethod = c(\"escape\", \"double\"),fileEncoding = \"\")\n" +
	"	write.table(output, file = paste0(paste(name_test,paste(paste(name_cont,collapse=\"-\"), \"regions\", sep=\"_\"),sep=\"_\"),\".txt\"), append = FALSE, quote = TRUE, sep = \"\\t\",eol = \"\\n\", na = \"\", dec = \",\", row.names = F,col.names = TRUE, qmethod = c(\"escape\", \"double\"),fileEncoding = \"\")\n" +
	"}\n" +
        "#------------------------------------------------------------------------------\n" +
	...
	"	file <- paste(name_test, \"output.xlsx\",sep=\"_\")\n" +
	...
	"	for(k in listgns){\n" +
	"		cairo_pdf(paste(c(k,\"-\", name_test, \".pdf\"), collapse=\"\"))\n" +
	"		auxtest <- test[test[,4]==k,]\n" +
	...
	"}\n" +
        "#------------------------------------------------------------------------------\n" +
        "result <- ppal(fvcf, per=c(per_del,per_dup), per_gene=c(per_gene_del, per_gene_dup), cov_cont, name_test, name_cont, name_fasta, excel, genes, polymorphic, poly_regions, do_plot, restr, fixed, down)\n" +
    ...
    public int execute (String per_dup, String per_del, String per_gene_dup, String per_gene_del, String cov_cont, 
            String fvcf, String excel, String genes, String restr, String fixed, String down, String do_plot, String userDB, String passwordDB, String wd, 
            String name_test, String name_fasta, String[] name_cont, String polymorphic, String poly_regions) {
        // Start R session.
        ...
        re.eval(rCode);
        //print=true;
        REXP result = re.eval("scriptR(per_dup, per_del, per_gene_dup, per_gene_del, cov_cont, fvcf, excel, genes, restr, fixed, down, do_plot, userDB, passwordDB, wd, name_test, name_fasta, name_cont, polymorphic, poly_regions)");
        re.jriFlushConsole();
        print=false;
        //re.end();
    ...
    public void rBusy(Rengine rngn, int i) {
        System.out.println("rBusy: Not supported yet.");
        //throw new UnsupportedOperationException("rBusy: Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public String rReadConsole(Rengine rngn, String string, int i) {
        System.out.println("GRIDD rReadConsole = \n" + string);
        //throw new UnsupportedOperationException("rReadConsole: Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        return "";
    }
    ...
    public String rChooseFile(Rengine rngn, int i) {
        System.out.println("rChooseFile: Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        //throw new UnsupportedOperationException("rChooseFile: Not supported yet."); //To change body of generated methods, choose Tools | Templates.
        return "";
    }

    @Override
    public void rFlushConsole(Rengine rngn) {
        System.out.println("rFlushConsole: Not supported yet.");
        //throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public void rSaveHistory(Rengine rngn, String string) {
        System.out.println("rSaveHistory: Not supported yet.");
        //throw new UnsupportedOperationException("rSaveHistory: Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }

    @Override
    public void rLoadHistory(Rengine rngn, String string) {
        System.out.println("rLoadHistory: Not supported yet.");
        //throw new UnsupportedOperationException("rLoadHistory: Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
